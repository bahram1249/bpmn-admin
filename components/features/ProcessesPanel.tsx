...
export function ProcessesPanel({ variant = 'page', showToolbar = true, className = '', modalZIndex = 60 }: ProcessesPanelProps) {
    ...
    <div className={"space-y-3 " + (className || '')}>
          ...
          { key: 'staticId', header: 'StaticId' },
          {
            ...
            render: (p: any) => (
              <div className="flex items-center justify-end gap-2">
                <Button variant="secondary" size="sm" leftIcon={<List size={16} />} onClick={() => setActivitiesProcess(p)}>
                  Activities
                </Button>
                <Button variant="secondary" size="sm" leftIcon={<GitBranch size={16} />} onClick={() => setGraphProcess(p)}>
                  Graph
                </Button>
      ...
      {activitiesProcess && (
        <Modal
          ...
          zIndexClass="z-[60]"
        >
          <ActivitiesPanel fixedProcessId={activitiesProcess.id} modalZIndex={(modalZIndex || 60) + 10} variant="modal" />
        </Modal>
      )}
