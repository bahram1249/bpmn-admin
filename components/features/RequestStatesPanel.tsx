...
export interface RequestStateItem {
  id: number;
  requestId: number;
  activityId: number;
  userId?: number;
...
export type RequestStatesPanelProps = {
  variant?: 'page' | 'modal';
  showToolbar?: boolean;
  className?: string;
  modalZIndex?: number;
...
export function RequestStatesPanel({ variant = 'page', showToolbar = true, className = '', modalZIndex = 50 }: RequestStatesPanelProps) {
  ...
  async function load() {
    ...
    try {
      ...
      const res = await fetchJson<{ result: RequestStateItem[]; total: number }>(`/api/bpmn/request-states${q}`);
      setItems(res.result);
      setTotal(res.total);
    } catch (e: any) {
      setError(e.message);
